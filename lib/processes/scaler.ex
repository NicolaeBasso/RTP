defmodule SSE.Process.Scaler do
  ...
  def handle_info(:time_trigg, [cur_tweets, scaler_proc, worker_sup, worker_type]) do
    ...
    IO.puts(
      "current #{Atom.to_string(worker_type)} workers for #{worker_sup} supervisor:#{children.active}"
    )

    IO.puts(
      "current #{Atom.to_string(worker_type)} tweets for #{worker_sup} supervisor:#{cur_tweets}\n\n"
    )
  ...
  def set_workers(quantity, scaler_proc, worker_sup, worker_type)
      when quantity > 0 and worker_type == :sentiment do
    SSE.Supervisor.SentimentWorker.add_child(scaler_proc, worker_sup)
    set_workers(quantity - 1, scaler_proc, worker_sup, worker_type)
  end

  def set_workers(quantity, scaler_proc, worker_sup, worker_type)
      when quantity > 0 and worker_type == :engagement do
